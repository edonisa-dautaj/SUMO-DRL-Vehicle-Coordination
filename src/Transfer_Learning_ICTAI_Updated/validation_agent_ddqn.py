...
def validate_ddqn_agent(agent_id, sumo_config, vehicle_id, model_path, save_metric, edges_list, color, episodes=100, sumo_gui=True):
    ...
    # Initialize the DDQN agent (same architecture as used during training)
    ...
    for episode in range(episodes):
        ...
        if vehicle_id not in traci.vehicle.getIDList():
            try :
                ...
                if route_id not in traci.route.getIDList():
                    traci.route.add(route_id, edges_list)  # Define the route
                traci.vehicle.add(vehicle_id, routeID=route_id, depart="now")  # Reinsert the vehicle
        ...
        while not done:
            ...
            # Only exploitation during validation (no random exploration)
    ...
    print(f"DDQN validation completed for {agent_id}.")