...
import torch
from Collisions import Collisions_veh  # Ajouté pour la détection de collision


def one_hot_state(state, num_states=5):
    vec = np.zeros(num_states)
    vec[int(state)] = 1
    return vec


def train_dqn(agent_id, sumo_config, vehicle_id, save_model, save_metric, edges_list, color, episodes=1000, sumo_gui=False):
    ...
    for episode in range(episodes):
        ...
        # Réinsertion si le véhicule n'existe plus
        ...
        state_one_hot = env.get_DQN_state()
        done = False
        truncated = False
        total_reward = 0
        step_count = 0
        ...
        while not done and not truncated:
            ...
            # Collision ou dépassement de distance minimale
            ...
            # Décélérations des suiveurs
            num_affected, avg_decel = env.get_followers_deceleration(vehicle_id)

            next_state_vec = env.get_DQN_state()
            agent.store(state_one_hot, action, reward, next_state_vec, done or truncated)
            agent.train_step()

            state_one_hot = next_state_vec
            total_reward += reward
            step_count += 1
    ...
    print(f"Modèle DQN sauvegardé : {save_model}")
